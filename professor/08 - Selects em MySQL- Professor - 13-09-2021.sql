...
/* Mosttra os registros da tabela filmes em ordem decrescente de título. */
...
/* Mostra apenas as ocorrência distintas de pais na tabela filmes. */
...
/* Mostra os registros agrupados por país, contando os registros. */
...
select ano, count(titulo) as total from filmes
group by ano
order by total, ano;
...
/* Mostra a duração média dos filmes por gênero. */
select genero, avg(duracao) as duracao_media from filmes
group by genero
order by genero;

/* Mostra a duração média dos filmes por gênero, exceto filmes de terror e com média de duração maior que 100. */
select genero, avg(duracao) as duracao_media from filmes
where genero != 'Terror'
group by genero
having duracao_media > 100
order by genero;
